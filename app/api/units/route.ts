...
export async function POST(req: Request) {
  try {
    ...
    const { success } = await createUnit(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const { success } = await updateUnit(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = unitIdSchema.parse({ id });
    const { success } = await deleteUnit(validatedParams.id);
